...
function ResetDataCard() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        setError(json.error || "Failed to reset data.");
        return;
      }
      setDone(
        `Deleted ${json.deleted?.residents ?? 0} resident${json.deleted?.residents === 1 ? "" : "s"} and ${json.deleted?.users ?? 0} staff/user account${json.deleted?.users === 1 ? "" : "s"}. Admins were kept.`
      );
      setPassword("");
      setOpen(false);
    } catch {
  ...
  return (
    <div className="rounded-[24px] border border-red-200 bg-red-50/40 p-4 sm:p-5">
      <div className="flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-red-100 text-red-600">
          <Trash2 className="h-5 w-5" />
        </div>
        <div>
          <h3 className="text-lg font-bold text-red-700 sm:text-xl">Danger Zone</h3>
          <p className="text-sm text-red-500/90">
            Delete all residents and staff/user accounts. Super admins and barangay admins remain.
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-red-300 bg-white px-5 py-3 text-sm font-bold text-red-600 shadow-sm transition hover:bg-red-50"
      >
        <Trash2 className="h-4 w-4" />
        Reset Residents &amp; Staff
      </button>

      {done && (
        <p className="mt-3 rounded-xl bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">{done}</p>
      )}
        ...
        >
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-black text-slate-900">Delete all residents &amp; staff?</h3>
            </div>
            <p className="mt-3 text-sm font-semibold text-slate-600">
              This permanently deletes <strong>every resident</strong> and <strong>every staff/user account</strong> (and all their records). Super admins and barangay admins are kept. <strong>This cannot be undone.</strong>
            </p>
