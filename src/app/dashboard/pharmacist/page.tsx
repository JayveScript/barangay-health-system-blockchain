...
export default function PharmacistDashboardPage() {
  const [activeTab, setActiveTab] = useState<"overview" | "personal" | "residents" | "scan-qr">(
    "overview"
  );
  ...
  useEffect(() => {
    async function loadUser() {
      try {
        ...
        if (!res.ok || data.role !== "PHARMACIST") {
        ...
        console.error("PHARMACIST_DASHBOARD_ERROR", error);
        window.location.href = "/login";
      } finally {
        setLoading(false);
      }
  ...
  if (loading) {
    return <PortalLoader label="Loading Pharmacist dashboard..." />;
  }
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-white/70">Pharmacist Dashboard</p>
              </div>
            </div>
            <button
              onClick={() => setMobileSidebarOpen(false)}
          ...
          <div className="flex min-h-0 flex-1 flex-col rounded-[26px] border border-white/10 bg-white/5 p-5 backdrop-blur">
            <div className="mb-6 flex shrink-0 items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-white/70">Pharmacist Dashboard</p>
              </div>
            </div>
...
function PersonalInfoTab({
  ...
  return (
    <div className="space-y-5 pb-4">
      <Panel icon={<UserRound className="h-5 w-5" />} title="Personal Information" subtitle="Your pharmacist account details">
