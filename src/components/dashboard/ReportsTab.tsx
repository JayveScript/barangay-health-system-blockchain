...
import { FileBarChart2, Baby, RefreshCw } from "lucide-react";
...
const REPORTS = [
  { id: "maternal", label: "Maternal Summary", icon: <Baby className="h-4 w-4" /> },
] as const;

export function ReportsTab() {
  const [active, setActive] = useState<(typeof REPORTS)[number]["id"]>("maternal");
  const [data, setData] = useState<MaternalReport | null>(null);
  ...
  const load = async () => {
    try {
      setLoading(true);
      setError("");
      const res = await fetch("/api/reports/maternal", { cache: "no-store" });
        ...
        <div className="overflow-hidden rounded-[28px] border border-[#BFDBFE] bg-gradient-to-br from-[#0F172A] to-[#1E3A8A] p-5 text-white shadow-sm sm:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <h2 className="text-2xl font-black leading-tight">
                  Maternal Health Care Summary
                </h2>
                <p className="mt-0.5 text-sm text-white/80">
                  DOH FHSIS · BHS SUMTAB — Prenatal, Intrapartum & Postpartum
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              ...
              <ExportPdfButton fileName="maternal-summary" />
            </div>
          </div>
          ...
          {data && (
            <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
              ...
              <StatTile label="Maternal Records" value={String(data.totalRecords)} />
              ...
              <table className="w-full min-w-[640px] border-collapse text-sm">
                <thead>
                  <tr className="bg-[#EFF6FF] text-[#1E3A8A]">
                    ...
                    <th className="px-3 py-3 text-center text-xs font-black uppercase tracking-wide">10–14</th>
                    <th className="px-3 py-3 text-center text-xs font-black uppercase tracking-wide">15–19</th>
                    <th className="px-3 py-3 text-center text-xs font-black uppercase tracking-wide">20–49</th>
                    <th className="px-3 py-3 text-center text-xs font-black uppercase tracking-wide">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {data.rows.map((row, i) => {
